...
const monsterHuntWhitelist = [farmMonster[0], "goo", "bee", "crab", "croc", "armadilo", "snake", "crab", "squig", "frog", "tortoise", "minimush", /*"spider"*/];
// In game names
...
function masterMerchant() {
    if (character.name == merchantName) {
        ...
        if (character.map == potionSeller.map) { // If we are on the same map as the potion seller
            let distance = distanceToPoint(potionSeller.x, potionSeller.y, character.real_x, character.real_y);
...
function buyPotions() {
    let mp = potionTypes[1];
    let hp = potionTypes[0];
    let stack = potionTypes[2];
...
function buyUpgradeScrolls() {
    ...
    let npc = getNpcById('scrolls');
    let distance = distanceToPoint(npc.x, npc.y, character.real_x, character.real_y);
...
function startFarmers() {
    ...
    for (let i in farmerNames) {
        let farmer = farmerNames[i]; // Define each farmer
        if (farmer) {
...
function sendItemsToMerchant() {
    ...
    if (merchant != null) { // Is the merchant around?
        let distance = distanceToPoint(merchant.real_x, merchant.real_y, character.real_x, character.real_y);
        if (distance <= 300) {
...
function sendGoldToMerchant() {
    let retain = retainGoldAmount(); // Gold we want farmers to retain
    if (character.gold > retain) {
        let sendAmount = character.gold - retain;
...
function farmNormally() {
    ...
    let target = get_targeted_monster(); // If we have a target, define it
    ...
    let desiredMonster = get_nearest_monster({ type: farmMonster[0], no_target: true });
...
function handleMonsterHunts() {
    let currentServer = parent.server_region + ' ' + parent.server_identifier;
    ...
    if (character.s.monsterhunt == undefined && currentServer == mainServer) { // If we do not have a quest and we are on main server
        ...
        let server = character.s.monsterhunt.sn; // Example: "US III"
        let monster = character.s.monsterhunt.id; // Example "mummy"
        let amount = character.s.monsterhunt.c; // Example 5
        let time = character.s.monsterhunt.ms; // Example 1768677 milliseconds
        ...
        if (monsterHuntWhitelist.includes(monster)) {
            ...
            if (amount > 0) {
                ...
                if (target) {
                    ...
                    let desiredMonster = get_nearest_monster({ type: monster, no_target: true });
...
function attackMonsters(target) {
    ...
    if (target) {
        let distance = distanceToPoint(target.real_x, target.real_y, character.real_x, character.real_y);
        // If we can attack it
...
async function handleMerchantLocationIdle() {
    let location = merchantIdle[1]; // Check the letiable to see how we tell them where to "idle"
    if (character.map != location.map && !smart.moving) {
        await smart_move(location);
    } else {
        let distance = distanceToPoint(location.x, location.y, character.real_x, character.real_y);
...
function handleParty() {
    if (character.name == merchantName) {
        ...
        if (Object.keys(parent.party).length < partyNames.length) {
            ...
            for (let i in partyNames) {
                let player = partyNames[i]; // Define each member in the array
            ...
            if (exchangeWhitelist.includes(item.name)) {
                ...
                if (distance == null || (distance != null && distance >= 300)) {
                    if (!smart.moving) {
                        ...
                        let location = { x: npc.x, y: npc.y, map: npc.map };
                        smart_move(location);
                    }
...
function hunterSkills() {
    ...
    setInterval(function () {
        let targets = Object.values(parent.entities).filter(entity => entity.mtype === farmMonster[1] && is_in_range(entity, "3shot"));
...
function serverLogic(server, newRegion, newServerNumber) {
    let hour = 3600000; // One hour in milliseconds
    let minute = 60000; // One minute in milliseconds
    // Server we are on
    let currentServer = parent.server_region + ' ' + parent.server_identifier;
